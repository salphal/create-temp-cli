...
sshClient.on('ready', () => {
  ...
  sshClient.forwardOut(
    bastionMachineSSHConfig.host,
    bastionMachineSSHConfig.port,
    innerServerSSHConfig.host,
    innerServerSSHConfig.port,
    (err, stream) => {
      if (err) {
      ...
      innerSSHClient.connect({
        ...
        username: innerServerSSHConfig.username,
        password: innerServerSSHConfig.password,
      });
    }
  );
});

sshClient.connect(bastionMachineSSHConfig);
